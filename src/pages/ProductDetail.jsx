  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          {selection.motoModel && product.imagesByModel?.[selection.motoModel] ? (
            <div>
              <img onClick={() => setLightbox(product.imagesByModel[selection.motoModel])} src={product.imagesByModel[selection.motoModel]} alt={`${product.name} - ${selection.motoModel}`} className="w-full h-64 object-cover rounded cursor-pointer" />
              <p className="mt-2 text-xs text-accent">Imagen específica para {selection.motoModel}</p>
            </div>
          ) : (
            <div>
              <div onClick={() => setLightbox((product.images && product.images[0]) || '')} className="h-64 bg-cover rounded cursor-pointer" style={{ backgroundImage: `url('${(product.images && product.images[0]) || 'https://placehold.co/600x400/1a1a1a/e63312?text=RMP+Motos'}')` }} />
              {product.images && product.images.length > 1 && (
                <div className="mt-2 flex gap-2">
                  {product.images.map((img, idx) => (
                    <img key={idx} onClick={() => setLightbox(img)} src={img} alt={`${product.name} ${idx+1}`} className="w-20 h-12 object-cover rounded border border-[#222] cursor-pointer" />
                  ))}
                </div>
              )}
            </div>
